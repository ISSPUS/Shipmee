	...
	public Object reconstruct(ActorForm actorForm, BindingResult binding) {
		...
		if (!actorService.checkLogin() ||
			...
			this.addBinding(binding, actorForm.getPassword().equals(actorForm.getRepeatedPassword()),
					"repeatedPassword", "user.passwordMismatch");
			this.addBinding(binding, actorForm.getPassword().length() > 5,
					"password", "javax.validation.constraints.Min.message");
			this.addBinding(binding, !userNameInUse, "userName", "user.userName.inUse");
			...
			if (!actorService.checkLogin()){
				...
				this.addBinding(binding, actorForm.getAcceptLegalCondition(),
						"acceptLegalCondition", "user.rejectedLegalConditions");
...
//		}
	...
	private void addBinding(Errors errors, boolean mustBeTrue, String field, String validationError){
		if (!mustBeTrue){
			errors.rejectValue(field, validationError);
		}
	}
}
