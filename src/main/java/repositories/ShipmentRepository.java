...
public interface ShipmentRepository extends JpaRepository<Shipment, Integer> {
	...
	@Query("select s from Shipment s where ?1 like CONCAT('%',s.origin,'%') AND ?2 like CONCAT('%',s.destination,'%') AND (?3 IS NULL OR (year(s.departureTime) = year(?3) AND month(s.departureTime) = month(?3) AND day(s.departureTime) = day(?3)))"
