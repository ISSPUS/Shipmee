...
public interface RouteRepository extends JpaRepository<Route, Integer> {
	...
	@Query("select r from Route r where ?1 like CONCAT('%',r.origin,'%') AND ?2 like CONCAT('%',r.destination,'%') AND (?3 IS NULL OR (year(r.departureTime) = year(?3) AND month(r.departureTime) = month(?3) AND day(r.departureTime) = day(?3)))"
	...
	Page<Route> searchRoute(String origin, String destination, Date date, Date time, String envelope, String itemSize,Pageable page);
